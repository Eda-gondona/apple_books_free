...
app.config.update(config1.flask_config)
Session(app)
...
def after_request(response):
    ...
    # set CORS headers
    response.headers['Access-Control-Allow-Origin'] = config1.CORS_ALLOW_ORIGIN
...
class MyLogContextFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        record.ip = "local" if config1.IS_LOCAL_DEV else ip
        record.uid = uid
        return True
...
def init_logging():
    ...
    prefix = "PROD " if config1.IS_PRODUCTION else ""
